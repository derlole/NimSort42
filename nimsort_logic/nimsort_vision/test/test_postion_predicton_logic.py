...
class TestPositionPrediction:
    ...
    def test_random_position_update(self, predictor):
        ...
        random_x = uniform(0.06, 0.3)
        random_y = uniform(0.04, 0.07)
        random_z = 0.5

        predictor.set_object_data(object_type=0, position=[random_x, random_y, random_z], ts=1000)
    ...
    def test_threshold_removal(self, predictor):
        ...
        predictor.set_object_data(1, [0.56, 0.04, 0], 1000)
        ...
        assert predictor._over_threshold_objects[0].position[0] == pytest.approx(0.56)

    def test_threshold_removal_exact_boundary(self, predictor):
        """Test: Objekte genau auf dem Schwellwert (>= 0.55) werden entfernt."""
        predictor.set_conveyorbelt_speed(1.0)
    ...
    def test_all_objects_over_threshold_returns_sentinel(self, predictor):
        ...
        predictor.set_object_data(0, [0.56, 0.04, 0], 1000)

        result = predictor.calculate_next_object_position()
    ...
    def test_zero_conveyor_speed_allowed(self, predictor):
        """Test: Geschwindigkeit 0.0 ist erlaubt – keine Exception, Position bleibt gleich."""
        predictor.set_conveyorbelt_speed(0.0)
        predictor.set_object_data(0, [0.1, 0.04, 0.0], 1000)

        x, y, z, obj_type, obj_id = predictor.calculate_next_object_position()
    ...
    def test_negative_conveyor_speed_error(self, predictor):
        ...
        with pytest.raises(ValueError, match=r"\[WARN\]\[PoPr\]\[CNOP----\]"):
    ...
    def test_returned_id_matches_leading_object(self, predictor):
        """Test: Zurückgegebene ID gehört tatsächlich zum führenden Objekt."""
        predictor.set_conveyorbelt_speed(0.0)
        ...
        assert obj_id in predictor._objects
        assert predictor._objects[obj_id].position[0] == pytest.approx(x)

    # ── Remove by ID ──────────────────────────────────────────────────────────

    def test_remove_object_by_id(self, predictor):
        """Test: Objekt wird gezielt nach ID entfernt."""
        predictor.set_conveyorbelt_speed(0.0)

        predictor.set_object_data(0, [0.1, 0.04, 0], 1000)
        predictor.set_object_data(1, [0.25, 0.04, 0], 1000)

        _, _, _, _, leading_id = predictor.calculate_next_object_position()
        predictor.remove_object_by_id(leading_id)

        assert leading_id not in predictor._objects
        assert len(predictor._objects) == 1

    def test_remove_object_by_invalid_id(self, predictor):
        """Test: Entfernen einer nicht existierenden ID wirft keine Exception."""
        predictor.set_conveyorbelt_speed(0.0)
        predictor.set_object_data(0, [0.1, 0.04, 0], 1000)

        predictor.remove_object_by_id(999)  # ID existiert nicht
        assert len(predictor._objects) == 1  # Objekt bleibt erhalten

    def test_after_remove_next_object_becomes_leading(self, predictor):
        """Test: Nach Entfernen des führenden Objekts wird das nächste zurückgegeben."""
        predictor.set_conveyorbelt_speed(0.0)
